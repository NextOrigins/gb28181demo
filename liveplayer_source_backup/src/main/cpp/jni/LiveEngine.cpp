...
#include "liveEngine/ArLiveEngineEvent.h"
...
Java_io_anyrtc_live_internal_NativeInstance_nativeStartPush(JNIEnv *env, jobject obj,jlong nativePtr,jstring pushUrl) {
    IArLivePusher* arLivePushKit = reinterpret_cast<IArLivePusher *>(nativePtr);
    if (arLivePushKit != NULL){ std::string strPushUrl = webrtc::JavaToStdString(env, pushUrl);
...
void recvBroadcastCallback(uint8_t *buffer, int size)
{
    if (mObj != NULL && mId != NULL && mEnv != NULL)
    {
        jbyteArray pArray = mEnv->NewByteArray(size);
        mEnv->SetByteArrayRegion(pArray, 0, size, (jbyte *) buffer);
        mEnv->CallVoidMethod(mObj, mId, pArray, size);

        mEnv->DeleteLocalRef(pArray);
    }

}
JNIEXPORT void JNICALL
Java_io_anyrtc_live_internal_NativeInstance_nativeRecvBroadcast(JNIEnv *env, jobject thiz,
                                                                jlong nativePtr,
                                                                jstring url, jobject obj) {

    IArLivePusher* arLivePushKit = reinterpret_cast<IArLivePusher *>(nativePtr);
    if (arLivePushKit != NULL){
        std::string s_url = webrtc::JavaToStdString(env, url);
        arLivePushKit->recvBroadcast(s_url.c_str(), recvBroadcastCallback);

        jclass pJclass = env->GetObjectClass(thiz);
        jmethodID id = env->GetMethodID(pJclass, "onPcm", "([B, I)V");
...
JNIEXPORT jint JNICALL
Java_io_anyrtc_live_internal_NativeInstance_nativeReplay(JNIEnv *env, jobject thiz,jlong nativeHandle
) {
    jint result = -1;
    IArLivePlayer* arLivePlayKit = reinterpret_cast<IArLivePlayer *>(nativeHandle);
    if (arLivePlayKit != NULL){
        result = arLivePlayKit->rePlay();
    }
    return result;
}
