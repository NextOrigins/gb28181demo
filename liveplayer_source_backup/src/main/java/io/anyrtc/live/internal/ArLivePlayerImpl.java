...
public class ArLivePlayerImpl extends ArLivePlayer {
    ...
    @Override
    public int rePlay() {
        return nativeInstance.nativeReplay(nativeId);
    }
    ...
    @Override
    public int seekTo(int seconds) {
        return nativeInstance.nativeSeekTo(nativeId,seconds);
    }

    @Override
    public int setSpeed(float speed) {
        if (speed!=0.75f&&speed!=0.5f&&speed!=1.0f&&speed!=1.25f&&speed!=1.5f&&speed!=1.75f&&speed!=2.0f&&speed!=3.0f){
            return ArLiveCode.ARLIVE_ERROR_NOT_SUPPORTED;
        }
        return nativeInstance.nativeSetSpeed(nativeId,speed);
    }

    @Override
    public void setAudioDualMonoMode(int eMode) {
        nativeInstance.setAudioDualMonoMode(nativeId,eMode);
    }
