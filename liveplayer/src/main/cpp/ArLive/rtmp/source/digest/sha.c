...
int USHAInput(USHAContext *context, const uint8_t *bytes, unsigned int bytecount)
{
	...
	case SHA256: return SHA256Input((SHA256Context*)&context->ctx, bytes, bytecount);
...
int USHAFinalBits(USHAContext *context, uint8_t bits, unsigned int bit_count)
{
	...
	switch (context->whichSha) {
	case SHA1: return SHA1FinalBits((SHA1Context*)&context->ctx, bits, bit_count);
	case SHA224: return SHA224FinalBits((SHA224Context*)&context->ctx, bits, bit_count);
	case SHA256: return SHA256FinalBits((SHA256Context*)&context->ctx, bits, bit_count);
	case SHA384: return SHA384FinalBits((SHA384Context*)&context->ctx, bits, bit_count);
	case SHA512: return SHA512FinalBits((SHA512Context*)&context->ctx, bits, bit_count);
	default: return shaBadParam;
	}
}
...
int USHAResult(USHAContext *context, uint8_t Message_Digest[USHAMaxHashSize])
{
	...
	case SHA256: return SHA256Result((SHA256Context*)&context->ctx, Message_Digest);
